...
import uuid
from .const import DOMAIN, DEVICE_TYPE, DEFAULT_DEVICE_ID, DEFAULT_DEVICE_NAME, DEFAULT_RESET_TIMEOUT, DEFAULT_PORT, DEFAULT_PATH
...
class MPPTConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        if user_input is not None:
            client_id = "mqttpy_" + uuid.uuid4().hex[:8]
            user_input["client_id"] = client_id
